...
def fix_datecode_ocr(value):
    """T39: Fix Date Code OCR and extract only the real 8-char date code.

    Rule:
    - Uppercase everything
    - O/o -> 0
    - Extract pattern: 3 digits + A-Z + 4 digits
      Examples:
        AALDLEET625U5009 -> 625U5009
        XYZ642A2003TEST -> 642A2003
        666Z4999 -> 666Z4999

    If multiple date-like patterns are found, use the last one because garbage text
    is usually read before the actual date code in the Vendor Date ROI.
    """
    src = keep_alnum_upper_for_general(value)
    if not src:
        return "", False, False
    ...
    fixed = src.replace("O", "0")
    matches = DATECODE_REGEX.findall(fixed)

    if matches:
        extracted = matches[-1]
        return extracted, extracted != src, True

    # Keep the fixed text for debugging if no valid pattern is found.
    return fixed, fixed != src, False
...
root.title("AI Camera Station - Mock API - T40 TOC OCR Fix")
