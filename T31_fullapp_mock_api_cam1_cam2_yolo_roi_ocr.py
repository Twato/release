...
# =========================================================
APP_VERSION = "T31_MOCK_API_YOLO_ROI_OCR"
...
# - Inner Box N validates Vendor BOX ID against box_id_list[N-1].
# =========================================================
...
def get_expected_box_id(inner_no):
    """Expected Vendor BOX ID for Inner Box N from API box_id_list."""
    try:
        idx = int(inner_no) - 1
        if 0 <= idx < len(box_id_list):
            return box_id_list[idx]
    except Exception:
        pass
    return ""
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T31_MOCK_API_YOLO_ROI_OCR",
...
def build_inner_box_record(inner_no, cam1_data, cam2_data, process_dir=""):
    ...
    picking_box_id = get_expected_box_id(inner_no)
    if not picking_box_id and job_data:
        api_box_ids = job_data.get("picking", {}).get("picking_box_ids", []) or []
        try:
            if 0 <= int(inner_no) - 1 < len(api_box_ids):
                picking_box_id = api_box_ids[int(inner_no) - 1]
        except Exception:
            picking_box_id = ""
    ...
    if picking_box_id:
        ...
        fail_reason.append("RULE_1_EXPECTED_BOX_ID_MISSING_FROM_API")
    ...
    record = {
        ...
        "operator_en": en_no,
        "delivery_no": delivery_no,
        "expected_vendor_box_id": picking_box_id,
        ...
        "validation": {
            "rule_1_picking_box_id_eq_vendor_box_id": "PASS" if picking_box_id and "RULE_1_PICKING_BOX_ID_NOT_MATCH_VENDOR_BOX_ID" not in fail_reason and "RULE_1_EXPECTED_BOX_ID_MISSING_FROM_API" not in fail_reason else "FAIL",
...
def show_product_result(product_no, cam1_data, cam2_data):
    ...
    vendor_validation = f"Expected : {record.get('expected_vendor_box_id', '-') or '-'} | {record['validation']['rule_1_picking_box_id_eq_vendor_box_id']}"
...
root.title("AI Camera Station - Mock API - T31")
