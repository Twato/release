...
from camera_test_T42 import PiCameraTest, UsbCameraTest
import camera_test_T42 as camera_module
...
# =========================================================
APP_VERSION = "T47_MANUAL_QUANTITY_TEST"
...
def find_available_usb_device():
    ...
    for dev in USB_SCAN_DEVICES:
        ...
        try:
            ...
            try:
                # T39: probe USB camera using the same 2K fixed-focus mode as CAM3.
...
# =========================================================
# T42 CAM3 TOC MODEL + DETECTION HOLD TIME
# Purpose:
# - Use the newly trained TOC OBB model instead of the old TOC model.
# - Hold YOLO detection for 4 seconds before capture to prevent instant capture.
# - Patch camera_test_T42.UsbCameraTest so it can read both normal bbox and OBB results.
# =========================================================
CAM3_TOC_MODEL_PATH = os.path.join(APP_DIR, "train_toc_obb_v2.pt")
CAM3_TOC_MODEL_FALLBACK_PATHS = [
    os.path.join(APP_DIR, "models", "TOC_model", "train_toc_obb_v2.pt"),
    "/home/toto1/AI_camera/models/TOC_model/train_toc_obb_v2.pt",
    "/home/toto/AI_CAMERA_TEST_YOLO_ROI/train_toc_obb_v2.pt",
    os.path.join(APP_DIR, "TOC_V2.pt"),
]
CAM3_TOC_DETECTION_HOLD_TIME = 4.0


def resolve_cam3_toc_model_path():
    for path in [CAM3_TOC_MODEL_PATH] + CAM3_TOC_MODEL_FALLBACK_PATHS:
        try:
            if path and os.path.exists(path):
                return path
        except Exception:
            pass
    return CAM3_TOC_MODEL_PATH


try:
    _resolved_toc_model = resolve_cam3_toc_model_path()
    camera_module.TOC_MODEL_PATH = _resolved_toc_model
    camera_module.USB_HOLD_TIME = CAM3_TOC_DETECTION_HOLD_TIME
    # Keep config_test unchanged; this overrides the imported globals inside camera_test_T42.py at runtime.
    print("T42 CAM3 TOC model applied:", _resolved_toc_model)
    print("T42 CAM3 detection hold time:", CAM3_TOC_DETECTION_HOLD_TIME)
except Exception as e:
    print("T42 CAM3 model/hold apply warning:", e)


def _t42_obb_points_to_xyxy(points):
    xs = [float(p[0]) for p in points]
    ys = [float(p[1]) for p in points]
    return [min(xs), min(ys), max(xs), max(ys)]


def _t42_detect_toc_rois(self, frame):
    """Detect TOC ROIs from either YOLO bbox or YOLO OBB model output."""
    if getattr(self, "model", None) is None:
        return []

    results = self.model(
        frame,
        conf=getattr(camera_module, "YOLO_CONF", 0.25),
        imgsz=getattr(camera_module, "YOLO_IMGSZ", 640),
        verbose=False,
    )

    expected = set(getattr(camera_module, "EXPECTED_TOC_CLASSES", []))
    order = getattr(camera_module, "TOC_ORDER", {})
    detections = []

    for r in results or []:
        names = getattr(self.model, "names", getattr(self, "names", {})) or getattr(r, "names", {}) or {}

        # New TOC model can be OBB. Read result.obb first.
            ...
            for i in range(n):
                try:
                    ...
                    if getattr(obb, "xyxy", None) is not None:
                        ...
                        xyxy = _t42_obb_points_to_xyxy(obb_points[i])
                    else:
                        continue
                    ...
                    print("T42 TOC OBB parse warning:", e)
            # Do not continue; some ultralytics results may expose both, but OBB is preferred.
            if n:
                continue

        # Backward-compatible bbox model support.
        ...
        for box in boxes:
            try:
                ...
                print("T42 TOC bbox parse warning:", e)

    # If duplicate class appears, keep highest confidence.
    best_by_class = {}
...
try:
    UsbCameraTest._detect_toc_rois = _t42_detect_toc_rois
    print("T42 UsbCameraTest patched for TOC OBB/bbox detection")
except Exception as e:
    print("T42 TOC OBB patch warning:", e)
...
# T42 fixed-focus settings for CAM1/CAM2.
# Tune LensPosition if label distance changes. Common test values: 4.0 / 5.0 / 6.0 / 7.0 / 8.0
PI_FIXED_FOCUS_ENABLE = True
PI_FIXED_FOCUS_CAM1 = 6.0
PI_FIXED_FOCUS_CAM2 = 6.0
PI_FIXED_FOCUS_SETTLE_SEC = 0.05
...
# =========================================================
# T37/T38 LOT NO OCR FIXER (TOA / TOB)
# Rule:
# - 10 chars total
# - First 3 chars     : digits only
# - Middle pattern    : TOA = U + digit + K, TOB = U + digit + C
# - Last 4 chars      : HEX only (0-9, A-F)
# Purpose:
# - Fix common OCR confusion after EasyOCR reads detected LOT NO ROI.
# - Example errors: O/o/Q/D -> 0, I/L/| -> 1, S -> 5, B -> 8.
# =========================================================
DIGIT_OCR_MAP = {
    "O": "0", "Q": "0", "D": "0", "o": "0",
    "I": "1", "L": "1", "l": "1", "|": "1", "!": "1",
...
HEX_OCR_MAP = {
    ...
    "S": "5", "$": "5",
    "G": "6",
    # B is valid HEX already, so keep B as B in HEX zone.
}
...
def keep_alnum_upper(value):
    return "".join(ch for ch in str(value or "").upper() if ch.isalnum())


def fix_digit_char(ch):
    ch = str(ch or "").upper()
    if ch.isdigit():
        return ch
    return DIGIT_OCR_MAP.get(ch, ch)
...
def fix_hex_char(ch):
    ...
    if ch in "0123456789ABCDEF":
        return ch
    return HEX_OCR_MAP.get(ch, ch)


def lot_window_score(window, lot_kind):
    """Score a 10-char OCR window. Higher score = better fit to LOT NO pattern."""
    if len(window) != 10:
        return -999

    lot_kind = str(lot_kind or "").upper()
    expected_mid2 = "C" if lot_kind == LOT_PREFIX_TOB else "K"
    score = 0
    ...
    if window[4].isdigit():
        score += 3
    elif fix_digit_char(window[4]).isdigit():
        score += 2
    else:
        score -= 2

    if window[5] == expected_mid2:
        score += 4
    else:
        # common OCR confusion for K/C is not reliable, so do not over-penalize.
        score -= 1
    ...
    for i in range(6, 10):
        ...
        if c in "0123456789ABCDEF":
            score += 3
        elif fix_hex_char(c) in "0123456789ABCDEF":
            score += 2
        else:
            score -= 2
...
def choose_lot_window(cleaned, lot_kind):
    cleaned = keep_alnum_upper(cleaned)
    if len(cleaned) <= 10:
        return cleaned
...
def fix_lot_no_ocr(value, lot_kind):
    """Fix LOT NO OCR text using known TOA/TOB pattern.

    Returns:
        fixed_text, changed_bool, is_pattern_valid
    """
    lot_kind = str(lot_kind or "").upper()
    expected_mid2 = "C" if lot_kind == LOT_PREFIX_TOB else "K"

    src = keep_alnum_upper(value)
    if not src:
        return "", False, False

    src = choose_lot_window(src, lot_kind)

    # Do not invent missing length. Keep fixed partial text for UI/debug.
    chars = list(src[:10])
    while len(chars) < 10:
        chars.append("")

    fixed = []
    for i, ch in enumerate(chars[:10]):
        if i <= 2:
            fixed.append(fix_digit_char(ch))
        elif i == 3:
            fixed.append("U")
        elif i == 4:
            fixed.append(fix_digit_char(ch))
        elif i == 5:
            fixed.append(expected_mid2)
        else:
            fixed.append(fix_hex_char(ch))

    fixed_text = "".join(fixed)
    valid = (
        len(fixed_text) == 10
        and fixed_text[:3].isdigit()
        and fixed_text[3] == "U"
        and fixed_text[4].isdigit()
        and fixed_text[5] == expected_mid2
        and all(c in "0123456789ABCDEF" for c in fixed_text[6:])
    )

    return fixed_text, fixed_text != keep_alnum_upper(value), valid


def fix_lot_no_by_field(value, field_key):
    field_key = str(field_key or "")
    if field_key == "toa_lot_no":
        return fix_lot_no_ocr(value, LOT_PREFIX_TOA)
    if field_key == "tob_lot_no":
        return fix_lot_no_ocr(value, LOT_PREFIX_TOB)
    return str(value or "").strip(), False, False


# =========================================================
# T38 OCR POST-PROCESSING FIXER
# Apply one centralized OCR fix before Result UI / Validation / JSON.
# - BOX ID     : uppercase + O/o -> 0
# - Date Code  : uppercase + O/o -> 0
# - LOT NO     : keep T37 pattern fixer for TOA/TOB
# =========================================================
def keep_alnum_upper_for_general(value):
    return "".join(ch for ch in str(value or "").upper() if ch.isalnum())


def fix_boxid_ocr(value):
    src = keep_alnum_upper_for_general(value)
    if not src:
        return "", False, False
    fixed = src.replace("O", "0")
    return fixed, fixed != src, bool(fixed)


DATECODE_REGEX = re.compile(r"\d{3}[A-Z]\d{4}")

# T41 Date Code OCR fixer
# Pattern: 8 chars total = 3 digits + 1 letter A-Z + 4 digits.
# The 4th position must be a letter. If OCR reads that position as a number,
# convert common OCR confusions back to a likely letter instead of leaving it as digit.
DATE_ALPHA_OCR_MAP = {
    # Common letter-position confusions
    "0": "U", "O": "U", "Q": "U", "D": "U",  # seen case: 62107000 should be 621U7000
    "1": "I", "I": "I", "L": "I", "|": "I", "!": "I",
    "2": "Z", "Z": "Z",
    "4": "A", "A": "A",
    "5": "S", "S": "S", "$": "S",
...
def fix_date_alpha_char(ch):
    """Return an A-Z char for Date Code position 4."""
    ch = str(ch or "").upper()
    if ch in DATE_ALPHA_OCR_MAP:
        return DATE_ALPHA_OCR_MAP[ch]
    if "A" <= ch <= "Z":
        return ch
    return ""


def date_window_score(window):
    """Score an 8-char OCR window for Date Code pattern DDD[A-Z]DDDD."""
    if len(window) != 8:
        return -999

    score = 0

    # Positions 0-2 and 4-7 must be digits.
    ...
    # Position 3 must be A-Z.
    c = window[3]
    if "A" <= c <= "Z":
        score += 4
    elif fix_date_alpha_char(c):
        score += 2
    else:
        score -= 4
...
def choose_datecode_window(src):
    """Choose the best 8-char window and convert it to DDD[A-Z]DDDD."""
    src = keep_alnum_upper_for_general(src)
    if not src:
        return "", False

    # 1) If a valid pattern already exists, use the last match.
    # This still fixes cases like AALDLEET625U5009 -> 625U5009.
    matches = DATECODE_REGEX.findall(src)
    if matches:
        return matches[-1], True

    # 2) Otherwise score every 8-char window and repair by position.
    if len(src) <= 8:
        best = src
        best_score = date_window_score(src.ljust(8, "_"))
    else:
        best = src[:8]
        best_score = -999
        for i in range(0, len(src) - 7):
            w = src[i:i + 8]
            s = date_window_score(w)
            if s >= best_score:
                # Use >= to prefer the later window because garbage text is often before the real date code.
                best = w
                best_score = s
    ...
    for i, ch in enumerate(chars):
        if i in (0, 1, 2, 4, 5, 6, 7):
            ...
            fixed.append(fix_date_alpha_char(ch))

    fixed_text = "".join(fixed)
    valid = bool(DATECODE_REGEX.fullmatch(fixed_text))
    return fixed_text, valid


def fix_datecode_ocr(value):
    """T41: Fix Date Code OCR and extract/repair only the real 8-char date code.

    Rule:
    - 8 chars total: 3 digits + A-Z + 4 digits
    - Digit positions fix common OCR confusions: O/Q/D -> 0, I/L -> 1, S -> 5, B -> 8, etc.
    - Letter position fixes common OCR confusions: 0/O/Q/D -> U, 1/I/L -> I,
      5 -> S, 8 -> B, 6 -> G, 2 -> Z, 4 -> A, 7 -> T.

    Examples:
      AALDLEET625U5009 -> 625U5009
      62107000          -> 621U7000
      642A2003          -> 642A2003
    """
    src = keep_alnum_upper_for_general(value)
    ...
    fixed_text, valid = choose_datecode_window(src)
    return fixed_text, fixed_text != src, valid

def fix_ocr_text_by_field(value, field_key):
    """Return fixed OCR text, changed flag, and optional pattern validity.

    This function is the single source for OCR cleanup.
    Result UI, validation, storage JSON, and search index all receive this fixed value.
    """
    field_key = str(field_key or "")

    if field_key == "vendor_box_id":
        return fix_boxid_ocr(value)

    if field_key in ("vendor_date_code", "toa_date_code", "tob_date_code"):
        return fix_datecode_ocr(value)

    if field_key in ("toa_lot_no", "tob_lot_no"):
        return fix_lot_no_by_field(value, field_key)

    src = keep_alnum_upper_for_general(value)
    return src, src != str(value or "").strip(), bool(src)



# =========================================================
# T40 TOC OCR POST-PROCESSING FIXER
# - TOC1-TOC6 use the same pattern as TOA LOT NO: DDD U D K HEXHEXHEXHEX
# - TOC main delivery code has 10 chars:
#     char 0     : only 0 or I
#     chars 1-9  : digits only
#   Common OCR fixes:
#     first char 1 -> I, O/o -> 0
#     digit chars O/Q/D -> 0, I/L/| -> 1, S -> 5, B -> 8
# =========================================================
def fix_toc_lot_ocr(value):
    """Fix TOC1-TOC6 OCR text using TOA LOT pattern."""
    return fix_lot_no_ocr(value, LOT_PREFIX_TOA)


def toc_delivery_window_score(window):
    """Score a 10-char OCR window for TOC delivery pattern: [0/I] + 9 digits."""
    if len(window) != 10:
        return -999

    score = 0

    # First char can only be 0 or I.
    c0 = str(window[0] or "").upper()
    if c0 in ("0", "I"):
        score += 5
    elif c0 == "1":
        # Common OCR issue: first char I read as 1.
        score += 4
    elif c0 in ("O", "Q", "D"):
        # Common OCR issue: 0 read as O/Q/D.
        score += 4
    else:
        score -= 3

    # Last 9 chars must be digits.
    for i in range(1, 10):
        c = str(window[i] or "").upper()
        if c.isdigit():
            score += 3
        elif fix_digit_char(c).isdigit():
            score += 2
...
def choose_toc_delivery_window(cleaned):
    cleaned = keep_alnum_upper(cleaned)
    if len(cleaned) <= 10:
        return cleaned
...
def fix_toc_delivery_ocr(value):
    """Fix TOC main OCR text.

    Pattern:
      - 10 chars total
      - first char only 0 or I
      - last 9 chars digits only
    """
    src = keep_alnum_upper(value)
    if not src:
        return "", False, False

    src_window = choose_toc_delivery_window(src)
    chars = list(src_window[:10])
    while len(chars) < 10:
        chars.append("")

    fixed = []

    # Position 0: only 0 or I.
    c0 = str(chars[0] or "").upper()
    if c0 == "1":
        fixed.append("I")
    elif c0 in ("O", "Q", "D"):
        fixed.append("0")
        ...
        # Keep unknown char for debug instead of silently inventing.
        fixed.append(c0)

    # Position 1-9: digits only.
    for i in range(1, 10):
        c = fix_digit_char(chars[i])
        fixed.append(c if str(c).isdigit() else c)

    fixed_text = "".join(fixed)
    valid = (
        len(fixed_text) == 10
        and fixed_text[0] in ("0", "I")
        and fixed_text[1:].isdigit()
    )
    return fixed_text, fixed_text != keep_alnum_upper(value), valid


def fix_toc_ocr_by_name(value, name):
    """Fix OCR from CAM3 TOC rows."""
    row = str(name or "").strip().lower()
    if row == "toc":
        return fix_toc_delivery_ocr(value)
    if row in ("toc1", "toc2", "toc3", "toc4", "toc5", "toc6"):
        return fix_toc_lot_ocr(value)
    src = keep_alnum_upper_for_general(value)
    return src, src != str(value or "").strip(), bool(src)
...
def process_inner_yolo_roi_ocr(image_path, camera_key):
    ...
    for idx, det in enumerate(detections, start=1):
        ...
        ocr_raw = ocr.get("raw", "")
        ocr_clean_original = ocr.get("clean", "")
        ocr_clean_fixed, ocr_fix_applied, ocr_pattern_valid = fix_ocr_text_by_field(ocr_clean_original, field_key)
        ...
        item = {
            ...
            "box": fixed_box,
            "raw": ocr_raw,
            "clean": ocr_clean_fixed,
            "ocr_clean_original": ocr_clean_original,
            "ocr_fix_applied": ocr_fix_applied,
            "ocr_pattern_valid": ocr_pattern_valid,
            "lot_pattern_valid": ocr_pattern_valid if field_key in ("toa_lot_no", "tob_lot_no") else None,
            "items": ocr.get("items", []),
        }
        by_class[name] = item
        if field_key:
...
quantity = 0        # Inner Box total from touchscreen Quantity input.
current_index = 0   # Current Inner Box index.
current_outer_index = 0
outer_total = 0     # ceil(quantity / 6)
inner_toa_lot_by_index = {}  # Inner Box no -> TOA Lot No OCR value
manual_job_ref = ""
...
picking_api_data = {}
box_id_list = []
completed_delivery_override = False
completed_delivery_override_attempt = ""


def normalize_scan_text(value, uppercase=False):
    """Normalize scanner/keyboard input before validation.

    Supported scanner suffix/prefix cleanup:
    - leading/trailing spaces
    - Enter / CR / LF
    - Tab

    Internal spaces are intentionally preserved so malformed data is rejected.
    """
    text = str(value or "")
...
def validate_en_value(value):
    """EN must be exactly 8 ASCII digits."""
    normalized = normalize_scan_text(value)
    return normalized, bool(re.fullmatch(r"[0-9]{8}", normalized))


def validate_delivery_value(value):
    """Delivery No must be exactly 10 uppercase alphanumeric characters."""
    normalized = normalize_scan_text(value, uppercase=True)
    return normalized, bool(re.fullmatch(r"[A-Z0-9]{10}", normalized))


def replace_entry_text(entry, value):
    """Replace Entry text without depending on scanner/keyboard source."""
    entry.delete(0, tk.END)
    entry.insert(0, value)


def focus_and_select(entry):
    """Focus the invalid field and select all so the next scan replaces it."""
    entry.focus_set()
...
def apply_picking_to_input_fields(data):
    ...
    try:
        ...
        delivery_entry.insert(0, normalize_scan_text(data.get("delivery_no", ""), uppercase=True))
...
def on_en_enter(event=None):
    """Scanner/keyboard Enter: validate EN, then move to Delivery No."""
    try:
        normalized, valid = validate_en_value(en_entry.get())
        replace_entry_text(en_entry, normalized)

        if not valid:
            messagebox.showwarning(
                "Invalid EN",
                "EN must contain exactly 8 numeric digits."
            )
            focus_and_select(en_entry)
            return "break"

        delivery_entry.focus_set()
        delivery_entry.selection_range(0, tk.END)
    except Exception as e:
        print("EN Enter validation error:", e)
    return "break"


def on_delivery_enter(event=None):
    """Scanner/keyboard Enter: validate Delivery No, then start the job."""
    try:
        normalized, valid = validate_delivery_value(delivery_entry.get())
        replace_entry_text(delivery_entry, normalized)

        if not valid:
            messagebox.showwarning(
                "Invalid Delivery No",
                "Delivery No must contain exactly 10 letters or digits."
            )
            focus_and_select(delivery_entry)
            return "break"

        start_btn.focus_set()
        start_job()
    except Exception as e:
        print("Delivery Enter validation error:", e)
    return "break"
...
# =========================================================
# T45 SUPERVISOR OVERRIDE CONFIG
# Purpose:
# - Normally block a Delivery No that already has attempt_xxx_completed.
# - Allow rework only when Operator EN is listed as supervisor/special EN.
# - Config is external so the EN list can be changed without editing code.
# =========================================================
SUPERVISOR_CONFIG_PATH = os.path.join(APP_DIR, "supervisor_config.json")
DEFAULT_SUPERVISOR_CONFIG = {
    "enabled": True,
    "supervisor_en": ["99999999", "88888888"],
    "allow_completed_delivery_rework": True,
    "note": "Supervisor EN can override completed Delivery No lock. Change these EN values for production."
}
    ...
    try:
        if not os.path.exists(SUPERVISOR_CONFIG_PATH):
            with open(SUPERVISOR_CONFIG_PATH, "w", encoding="utf-8") as f:
                json.dump(DEFAULT_SUPERVISOR_CONFIG, f, ensure_ascii=False, indent=2)
            return dict(DEFAULT_SUPERVISOR_CONFIG)

        with open(SUPERVISOR_CONFIG_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        if not isinstance(data, dict):
            return dict(DEFAULT_SUPERVISOR_CONFIG)

        merged = dict(DEFAULT_SUPERVISOR_CONFIG)
        merged.update(data)
        return merged
    except Exception as e:
        print("Supervisor config load error:", e)
        return dict(DEFAULT_SUPERVISOR_CONFIG)


def normalize_en_for_permission(value):
    return "".join(ch for ch in str(value or "").strip() if ch.isdigit())


def is_supervisor_en(value):
    cfg = load_supervisor_config()
    if not cfg.get("enabled", True):
        return False
    en_value = normalize_en_for_permission(value)
    allowed = {normalize_en_for_permission(x) for x in (cfg.get("supervisor_en") or [])}
    return bool(en_value and en_value in allowed)


def supervisor_override_allowed(reason="completed_delivery"):
    cfg = load_supervisor_config()
    if reason == "completed_delivery":
        return bool(cfg.get("allow_completed_delivery_rework", True))
    return False
...
def timestamp_for_filename():
    """Timestamp + short UUID for image/evidence filenames.
    This prevents duplicate filenames even if captures happen in the same microsecond.
    """
    return datetime.now().strftime("%Y%m%d_%H%M%S_%f") + "_" + uuid.uuid4().hex[:8]


def make_unique_storage_name(base_name):
    """Return a safe filename with timestamp+uuid before the extension."""
    base = os.path.basename(str(base_name or "evidence.jpg"))
    stem, ext = os.path.splitext(base)
    if not ext:
        ext = ".jpg"
    return f"{safe_folder_name(stem, 60)}_{timestamp_for_filename()}{ext.lower()}"


def copy_if_exists(src, dst_dir, new_name=None):
    """Copy evidence without risking duplicate filenames.
    If new_name is supplied, timestamp+uuid is still appended before extension.
    """
    if not src or not os.path.exists(src):
        return ""
    ensure_dir(dst_dir)
    filename = make_unique_storage_name(new_name or os.path.basename(src))
...
def make_job_id(delivery=None):
    ref = manual_job_ref or datetime.now().strftime("MANUAL_TEST_%Y%m%d_%H%M%S")
    return ref
...
def find_delivery_dirs(delivery):
    """Find all DELIVERY_<DeliveryNo> folders under storage, not only today.
    This lets the app block a Delivery No that was completed on a previous day.
    """
    results = []
    target = f"DELIVERY_{safe_folder_name(delivery)}"
    if not os.path.isdir(STORAGE_ROOT):
        return results
    try:
        for root_dir, dirs, _ in os.walk(STORAGE_ROOT):
            if os.path.basename(root_dir) in ("cleanup_logs",):
                dirs[:] = []
                continue
            for d in dirs:
                if d == target:
        ...
        print("Find delivery dirs error:", e)
    return sorted(set(results))


def find_completed_delivery_attempt(delivery):
    """Return the completed attempt path if this Delivery No has already been closed."""
    for ddir in find_delivery_dirs(delivery):
        try:
            for name in sorted(os.listdir(ddir)):
                if name.startswith("attempt_") and name.lower().endswith("_completed"):
...
def delivery_has_running_attempt(delivery):
    """Best-effort warning helper for an unfinished attempt of the same Delivery No."""
    for ddir in find_delivery_dirs(delivery):
        try:
            for name in sorted(os.listdir(ddir)):
                if name.startswith("attempt_") and name.lower().endswith("_running"):
                    return os.path.join(ddir, name)
        except Exception:
            continue
    return ""
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T47_MANUAL_QUANTITY_TEST",
        "mode": "MANUAL_QUANTITY_TEST",
        "mock_api_enabled": False,
        "manual_job_ref": manual_job_ref,
        ...
        "operator_en": en_no,
        "supervisor_override": {
            "used": bool(globals().get("completed_delivery_override", False)),
            "reason": "completed_delivery_rework" if bool(globals().get("completed_delivery_override", False)) else "",
            "supervisor_en": en_no if bool(globals().get("completed_delivery_override", False)) else "",
            "config_path": SUPERVISOR_CONFIG_PATH if bool(globals().get("completed_delivery_override", False)) else "",
            "time": now_iso() if bool(globals().get("completed_delivery_override", False)) else "",
            "previous_completed_attempt": globals().get("completed_delivery_override_attempt", "") if bool(globals().get("completed_delivery_override", False)) else "",
        },
        "picking": {
            "picking_delivery_no": "",
            "picking_item_no": "-",
            ...
            "picking_box_ids": [],
            "picking_api": {"mode": "MANUAL_QUANTITY_TEST", "mock_api_enabled": False},
        },
        "summary": {
...
def build_inner_box_record(inner_no, cam1_data, cam2_data, process_dir=""):
    ...
    # T44 storage cleanup:
    # - Full images stay only at process_attempt root (cam1/cam2 files already saved there).
    # - Vendor/TOA/TOB folders keep ROI/processed ROI only.
    # - JSON keeps a reference to the full image path for audit.
    evidence = {
        "vendor": {
            "full_image_ref": vendor_box_data.get("image_path", cam1_data.get("image_path", "")),
            "roi_image": copy_if_exists(vendor_box_data.get("roi_path", ""), vendor_folder, f"roi_vendor_boxid_{safe_folder_name(vendor_box_id)}.jpg"),
            "processed_image": copy_if_exists(vendor_box_data.get("processed_path", ""), vendor_folder, "processed_vendor_boxid.jpg"),
        },
        "toa": {
            "full_image_ref": toa_lot_data.get("image_path", cam1_data.get("image_path", "")),
            "roi_image": copy_if_exists(toa_lot_data.get("roi_path", ""), toa_folder, f"roi_toa_lot_{safe_folder_name(toa_lot_no)}.jpg"),
            "processed_image": copy_if_exists(toa_lot_data.get("processed_path", ""), toa_folder, "processed_toa_lot.jpg"),
        },
        "tob": {
            "full_image_ref": tob_lot_data.get("image_path", cam2_data.get("image_path", "")),
            "roi_image": copy_if_exists(tob_lot_data.get("roi_path", ""), tob_folder, f"roi_tob_lot_{safe_folder_name(tob_lot_no)}.jpg"),
            "processed_image": copy_if_exists(tob_lot_data.get("processed_path", ""), tob_folder, "processed_tob_lot.jpg"),
        },
    }
    ...
    record = {
        ...
        "operator_en": en_no,
        "delivery_no": delivery_no,
        "expected_vendor_box_id": "N/A_MANUAL_TEST",
        ...
        "validation_mode": "MANUAL_QUANTITY_DISPLAY_ONLY",
        "can_continue_even_if_fail": True,
        "fail_reason": fail_reason,
        "fail_reason_text": [validation_reason_label(x) for x in fail_reason],
        "validation": {
            "rule_1_vendor_box_id_in_api_list_any_order": "N/A",
            "rule_1_vendor_box_id_not_duplicate": "PASS" if "RULE_1_VENDOR_BOX_ID_DUPLICATE_IN_JOB" not in fail_reason else "FAIL",
            "rule_1_picking_box_id_eq_vendor_box_id": "N/A",
            "rule_1_vendor_box_id_read": "PASS" if "RULE_1_VENDOR_BOX_ID_OCR_EMPTY" not in fail_reason else "FAIL",
        ...
        "evidence": evidence,
        "full_images": {
            "cam1_vendor_toa": cam1_data.get("image_path", ""),
            "cam2_tob": cam2_data.get("image_path", ""),
        },
...
def build_outer_box_record(toc_no, toc_total, item_map, fallback_data=None, process_dir=""):
    ...
    fail_reason = []
    # Manual Quantity Test has no Delivery No target. TOC main is display-only.
    if unmatched_lots:
    ...
    for name, data in item_map.items():
        ...
        evidence[name] = {
            "full_image_ref": data.get("image_path", ""),
    ...
    if fallback_data is not None and "toc" not in evidence:
        ...
        evidence["toc_fallback"] = {
            "full_image_ref": fallback_data.get("image_path", ""),
    ...
    return {
        ...
        "fail_reason": fail_reason,
        "validation": {
            "rule_2_picking_delivery_no_eq_toc_delivery_no": "N/A",
        ...
        "evidence": evidence,
        "full_images": {
            "cam3_toc": (next(iter(item_map.values())).get("image_path", "") if item_map else (fallback_data or {}).get("image_path", "")),
        },
...
def now_name(prefix):
    # T44: add short UUID to guarantee no duplicate image filenames.
    safe_prefix = safe_folder_name(prefix, 50)
    return f"{safe_prefix}_{timestamp_for_filename()}.jpg"
...
class OptimizedPiCamera:
    ...
    def open(self):
        ...
        # OCR-friendly image tuning + T42 fixed focus.
        try:
            control_values = {
                "Sharpness": 1.2,
                "Contrast": 1.1,
            }
            if PI_FIXED_FOCUS_ENABLE and controls is not None:
                control_values["AfMode"] = controls.AfModeEnum.Manual
                control_values["LensPosition"] = PI_FIXED_FOCUS_CAM1 if self.name == "CAM1" else PI_FIXED_FOCUS_CAM2
            self.picam2.set_controls(control_values)
        except Exception as e:
            print(f"{self.name} fixed-focus/image control warning:", e)
    ...
    def apply_fixed_focus(self, keep_running=None):
        """T42: Keep lens in manual fixed-focus mode and avoid autofocus delay."""
        if self.picam2 is None:
            raise RuntimeError(f"{self.name} not opened")

        lens_position = PI_FIXED_FOCUS_CAM1 if self.name == "CAM1" else PI_FIXED_FOCUS_CAM2
        try:
            if PI_FIXED_FOCUS_ENABLE and controls is not None:
            ...
            print(f"{self.name} apply fixed focus warning:", e)

        end_settle = time.time() + max(0.0, PI_FIXED_FOCUS_SETTLE_SEC)
        while time.time() < end_settle:
            if keep_running is not None and not keep_running():
                break
            time.sleep(0.01)

        self._status(f"{self.name}: Fixed focus LensPosition={lens_position}")
        return PI_FIXED_FOCUS_SETTLE_SEC

    def autofocus(self, keep_running=None):
        # T42: autofocus is intentionally bypassed for speed and consistency.
        # Keep this method name for compatibility with older code paths.
        if PI_FIXED_FOCUS_ENABLE:
            return self.apply_fixed_focus(keep_running)
...
def set_status(text):
    global last_status_text, last_status_time

    current_time = time.time()

    # ��ͧ�ѹ��ͤ�������ѻവ UI ��Ӷ���Թ�
    if text == last_status_text and current_time - last_status_time < 0.5:
        return
    ...
    try:
        ...
        if capture_state_dot is not None:
            ...
            capture_state_dot.create_oval(
                2, 2, 12, 12,
                fill=color,
                outline=color
            )

        # ������� root.update_idletasks() �ç���

    except Exception as e:
        print("Status UI error:", e)

    print(text)

    if any(
        keyword in str(text).lower()
        for keyword in [
            "error",
            "reset",
            "opening",
...
def update_preview(frame, is_bgr=False):
    ...
    try:
        # T54: Fit preview inside the real HMI area so it cannot push the
        # Reset Current Camera button below the visible screen.
        target_w = max(80, preview_frame.winfo_width() - 8)
        target_h = max(60, preview_frame.winfo_height() - 8)
        h, w = frame.shape[:2]
...
current_visible_page = None


def show_page(page):
    global current_visible_page

    # �����˹����� ����ͧ pack_forget ���� pack ����
    if current_visible_page is page:
        return

    page_name = getattr(page, "_debug_name", str(page))
    dbg(f"show_page -> {page_name}")

    current_visible_page = page
...
def process_yolo_roi_ocr(image_path, detections):
    ...
    for det in detections:
        ...
        ocr_raw = ocr.get("raw", "")
        ocr_clean_original = ocr.get("clean", "")
        ocr_clean_fixed, ocr_fix_applied, ocr_pattern_valid = fix_toc_ocr_by_name(ocr_clean_original, name)
...
def make_result_field(parent, field_title, value_title):
    ...
    tk.Label(block, text=field_title, bg=CARD, fg=MUTED, font=("Arial", 9, "bold")).pack(anchor="w", pady=(0, 2))
    image_wrap = tk.Frame(block, bg=DARK, height=40)
    image_wrap.pack(fill="x", pady=(0, 3)); image_wrap.pack_propagate(False)
    img_box = tk.Label(image_wrap, bg=DARK, fg="white", text="NO IMAGE", font=("Arial", 8, "bold")); img_box.pack(fill="both", expand=True)
    ocr_row = tk.Frame(block, bg=CARD); ocr_row.pack(fill="x", pady=(0, 2))
    tk.Label(ocr_row, text=value_title, bg=CARD, fg=TEXT, font=("Arial", 8, "bold"), width=9, anchor="w").pack(side="left")
    value_box = tk.Label(ocr_row, text="-", bg="#f8f9fa", fg="#7f8c8d", font=("Arial", 10, "bold"), anchor="w", padx=4, pady=2)
    value_box.pack(side="left", fill="x", expand=True)
    validation_row = tk.Frame(block, bg=CARD); validation_row.pack(fill="x", pady=(0, 2))
    tk.Label(validation_row, text="Validate :", bg=CARD, fg=TEXT, font=("Arial", 8, "bold"), width=9, anchor="w").pack(side="left")
    validation_box = tk.Label(validation_row, text="WAIT", bg="#f8f9fa", fg="#7f8c8d", font=("Arial", 8, "bold"), anchor="w", padx=4, pady=2, wraplength=200, justify="left")
    validation_box.pack(side="left", fill="x", expand=True)
    status_badge = tk.Label(block, text="WAIT", bg="#f1c40f", fg=TEXT, font=("Arial", 8, "bold"), padx=7, pady=2)
    status_badge.pack(anchor="w")
    return {"image": img_box, "value": value_box, "validation": validation_box, "status": status_badge}



def make_product_result_section(parent, key, title, icon_key, field1, value1, field2, value2):
    card = tk.Frame(parent, bg=CARD, highlightbackground=BORDER, highlightthickness=1, padx=6, pady=5)
    card.pack(side="left", fill="both", expand=True, padx=3)
    title_row = tk.Frame(card, bg=CARD); title_row.pack(fill="x", pady=(0, 4))
    icon_label(title_row, icon_key, bg=CARD).pack(side="left", padx=(0, 4))
    tk.Label(title_row, text=title, bg=CARD, fg=TEXT, font=("Arial", 11, "bold")).pack(side="left")
...
def update_result_field(field, image_path, ocr_value, validation_text="WAIT", status="WAIT"):
    update_result_image(field["image"], image_path, size=(280, 40))
...
def get_toc_row_validation(name, clean_text):
    """Manual test validation for TOC result rows."""
    ...
    matched = "-"
    first_inner = ((max(1, current_outer_index) - 1) * 6) + 1
    last_inner = min(quantity, first_inner + 5)
    for inner_no, lot_value in sorted(inner_toa_lot_by_index.items()):
        if first_inner <= int(inner_no) <= last_inner and normalize_text_for_compare(lot_value) == read_norm:
            matched = lot_value
            break
...
def add_toc_result_row(parent, row_no, title, image_data=None):
    """Compact TOC result row for the small Station/HMI screen.

    Keeps all 7 rows visible on one page while preserving:
    ROI image, OCR result, RAW value, comparison target and PASS/NG status.
    """
    data = image_data or {}
    ...
    # Compact outer card: around 62-68 px per row so TOC + TOC1..TOC6 fit.
    row = tk.Frame(
        parent,
        bg=CARD,
    ...
    # Number + class name in a narrow fixed column.
    name_box = tk.Frame(row, bg=CARD, width=58, height=48)
    name_box.pack(side="left", fill="y", padx=(0, 5))
    ...
    # Smaller ROI while keeping enough width to inspect the text crop.
    roi_path = data.get("roi_path", "")
    photo = load_result_photo(roi_path, size=(185, 46))

    image_wrap = tk.Frame(row, bg=DARK, width=185, height=46)
    image_wrap.pack(side="left", padx=(0, 7))
    ...
    # Line 1: OCR value + raw/source, all preserved but compressed into one line.
    top_line = tk.Frame(info_box, bg=CARD)
    top_line.pack(fill="x")

    tk.Label(
        top_line,
        text=f"OCR: {clean_text}",
    ...
    tk.Label(
        top_line,
        text=f"RAW: {raw_text}  |  {source_text}",
        bg=CARD,
        fg=MUTED,
        font=("Arial", 7),
        anchor="e",
    ...
    # Line 2: read value, target comparison and result badge.
    validation_row = tk.Frame(info_box, bg=CARD)
    validation_row.pack(fill="x", pady=(2, 0))

    tk.Label(
        validation_row,
        text=f"Read: {validation['read']}",
        bg=CARD,
        fg=TEXT,
        font=("Arial", 8, "bold"),
        anchor="w",
    ...
    status = (validation.get("status") or "WAIT").upper()
    if status == "PASS":
        badge_text, badge_bg, badge_fg = "PASS", GREEN, "white"
    elif status in ("NG", "FAIL"):
        badge_text, badge_bg, badge_fg = "NG", RED, "white"
    else:
        badge_text, badge_bg, badge_fg = "WAIT", "#f1c40f", TEXT
...
def show_product_result(product_no, cam1_data, cam2_data):
    ...
    # T43 UI CLEANUP: keep validation visible inside cards only; hide top summary line.
    result_hint.config(text="")
    ...
    vendor_box_status = "PASS" if record["validation"].get("rule_1_vendor_box_id_read") == "PASS" and record["validation"].get("rule_1_vendor_box_id_not_duplicate") == "PASS" else "NG"
    ...
    vendor_validation = f"Manual Test | Read : {record['validation'].get('rule_1_vendor_box_id_read', 'WAIT')} | Duplicate : {record['validation'].get('rule_1_vendor_box_id_not_duplicate', 'WAIT')}"
...
def show_toc_result(toc_items, fallback_data=None, toc_no=1, toc_total=1):
    ...
    toc_result_area.pack(fill="both", expand=True, padx=4, pady=(0, 4))
    clear_toc_cards()
...
# =========================================================
def start_job():
    global running, en_no, delivery_no, quantity, current_index, current_outer_index, outer_total, item_no, inner_toa_lot_by_index
    global picking_api_data, box_id_list, completed_delivery_override, completed_delivery_override_attempt, manual_job_ref
    ...
    # Manual test mode: no EN, Delivery No, BOX ID list, or Mock API.
    manual_job_ref = datetime.now().strftime("MANUAL_TEST_%Y%m%d_%H%M%S")
    en_no = "MANUAL"
    delivery_no = ""
    item_no = "-"
    picking_api_data = {"mode": "MANUAL_QUANTITY_TEST", "mock_api_enabled": False}
    box_id_list = []
    completed_delivery_override = False
    completed_delivery_override_attempt = ""
    ...
    dbg("START MANUAL QUANTITY TEST JOB")
...
def process_product(product_no):
    ...
    while running:
        ...
        root.after(0, lambda: set_step("CAM1/CAM2 : Fixed-focus 2K no-switch capture"))
        root.after(0, lambda: set_status("CAM1/CAM2 fixed-focus optimized capture starting"))
        ...
        if USE_T21_OPTIMIZED_PI_CAMERAS:
            ...
            while running:
                ...
                try:
                    ...
                    # ===================== CAM1 =====================
                    root.after(0, lambda: setup_capture_page(mode="inner", active="TOA", done=[]))
                    root.after(0, lambda: set_step("CAM1 : Detect object by optimized 2K background (hold 4 sec)"))
                    root.after(0, lambda: set_status("CAM1 fixed-focus optimized opening/standby"))
                    ...
                    root.after(0, lambda: set_status("CAM2 fixed-focus optimized opening/standby"))
...
def restart_process():
    """Confirm before restarting only the current process/camera sequence."""
    dbg(f"Restart The Process pressed on {current_screen_mode}")
    log_event("RESTART_PROCESS_CONFIRM_OPENED", screen=current_screen_mode)

    confirmed = messagebox.askyesno(
        "Confirm Restart The Process",
        "Are you sure you want to restart the current process?\n\n"
        "The current Inner Box / Outer Box will be captured and read again.\n"
        "The job and current box number will not be cancelled.",
        icon="warning",
        parent=root,
    )

    if not confirmed:
        dbg("Restart The Process cancelled at confirmation popup")
        log_event("RESTART_PROCESS_CONFIRM_NO", screen=current_screen_mode)
        return

    dbg(f"Restart The Process confirmed on {current_screen_mode}")
    log_event("RESTART_PROCESS_CONFIRM_YES", screen=current_screen_mode)
...
def cancel_job():
    """Cancel the complete lot only after two explicit confirmations."""
    log_event("CANCEL_JOB_CONFIRM_1_OPENED", screen=current_screen_mode)
    dbg("Cancel Job pressed - confirmation 1")

    first_confirmed = messagebox.askyesno(
        "Confirm Cancel Job",
        "Are you sure you want to cancel this job?\n\n"
        "Press Yes to continue to the final confirmation.\n"
        "Press No to return to the current process.",
        icon="warning",
        parent=root,
    )

    if not first_confirmed:
        dbg("Cancel Job stopped at confirmation 1")
        log_event("CANCEL_JOB_CONFIRM_1_NO", screen=current_screen_mode)
        return

    log_event("CANCEL_JOB_CONFIRM_1_YES", screen=current_screen_mode)
    dbg("Cancel Job confirmation 1 accepted - opening final confirmation")

    second_confirmed = messagebox.askyesno(
        "FINAL CONFIRMATION - END LOT",
        "WARNING: This is the final confirmation.\n\n"
        "If you press Yes, the entire lot will be ended and saved as CANCELLED.\n"
        "You will return to the input screen and this action cannot be undone.\n\n"
        "Do you really want to end this lot?",
        icon="warning",
        parent=root,
    )

    if not second_confirmed:
        dbg("Cancel Job stopped at final confirmation")
        log_event("CANCEL_JOB_CONFIRM_2_NO", screen=current_screen_mode)
        return

    log_event("CANCEL_JOB_CONFIRM_2_YES", screen=current_screen_mode)
    log_event("CANCEL_JOB_REQUESTED")
    dbg("Cancel Job confirmed twice - ending complete lot")
    ...
    if job_data:
        ...
        job_data.setdefault("events", []).append({
            ...
            "event": "JOB_CANCELLED",
            "status": "CANCELLED",
            "cancel_reason": "operator_cancel_confirmed_twice",
        })
        finalize_attempt_status("CANCELLED", "operator_cancel_confirmed_twice")
    set_action("cancel")
    root.after(0, reset_to_input)
...
def reset_to_input():
    ...
    global completed_delivery_override, completed_delivery_override_attempt, manual_job_ref
...
def preload_yolo_model_if_available():
    ...
    """
    # 1) Preferred: explicit preload function in camera_test_T42.py.
    for fn_name in [
        "preload_yolo",
        "preload_yolo_model",
        "load_yolo",
        ...
        if callable(fn):
            fn()
            return f"OK via camera_test_T42.{fn_name}()"
...
root.title("AI Camera Station - T47 Button Confirmation")
...
input_card.place(relx=0.5, rely=0.50, anchor="center", relwidth=0.88, relheight=0.88)

tk.Label(input_card, text="Manual Quantity Test", font=("Arial", 18, "bold"), bg=CARD, fg=TEXT).pack(anchor="w", pady=(0, 8))
tk.Label(input_card, text="Quantity / Inner Box", bg=CARD, fg=TEXT, font=("Arial", 13, "bold")).pack(anchor="w", pady=(0, 5))

qty_entry = tk.Entry(input_card, font=("Arial", 24), relief="solid", borderwidth=1, justify="center")
qty_entry.pack(fill="x", ipady=6, pady=(0, 8))

# Hidden compatibility entries. Manual mode does not read EN or Delivery No.
en_entry = tk.Entry(input_card)
delivery_entry = tk.Entry(input_card)
...
keypad_frame.pack(fill="both", expand=True, pady=(0, 8))
...
for r, row in enumerate(key_rows):
    ...
    for c, key in enumerate(row):
        ...
        if key == "CLR":
            cmd, key_bg, key_fg = qty_clear, "#bfc3c7", TEXT
        elif key == "DEL":
            cmd, key_bg, key_fg = qty_backspace, "#bfc3c7", TEXT
        else:
            cmd, key_bg, key_fg = (lambda v=key: qty_insert(v)), SOFT, TEXT
        tk.Button(
            keypad_frame,
            text=key,
            font=("Arial", 17, "bold"),
...
preload_label.pack(fill="x", pady=(0, 7))

start_btn = tk.Button(input_card, text="START TEST", font=("Arial", 14, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=start_job)
start_btn.pack(fill="x", ipady=8, pady=(2, 6))
tk.Button(input_card, text="EXIT APP", font=("Arial", 14, "bold"), bg=RED, fg="white", activebackground=RED_DARK, activeforeground="white", relief="flat", command=exit_app).pack(fill="x", ipady=8)

qty_entry.bind("<Return>", lambda event: start_job())
...
tk.Label(search_header, text="Search / Audit Lookup", font=("Arial", 13, "bold"), bg=BG, fg=TEXT).pack(side="left")
...
tk.Button(search_row, text="SEARCH", font=("Arial", 12, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=audit_search_now, padx=22, pady=7).pack(side="left", padx=(0, 8))
tk.Button(search_row, text="OPEN PATH", font=("Arial", 12, "bold"), bg="#27ae60", fg="white", activebackground="#1e8449", activeforeground="white", relief="flat", command=audit_open_selected_path, padx=18, pady=7).pack(side="left")
...
# Section 1 header
header_frame = tk.Frame(capture_container, bg=CARD, highlightbackground=BORDER, highlightthickness=1, padx=10, pady=7)
...
# T43 UI CLEANUP:
# Capture page header keeps only process-level actions.
# Reset Camera remains under Preview. Result progression is automatic after validation PASS.
add_action_button(header_buttons, "Restart The Process", bg="#bfc3c7", fg=TEXT, command=restart_process, width_px=155, height_px=42)
add_action_button(header_buttons, "Cancel Job", bg=RED, fg="white", command=cancel_job, width_px=155, height_px=42)
...
inner_box_container.pack(side="left", padx=(0, 22))
...
info_box_title = tk.Label(info_box_text, text="Inner Box", bg=CARD, fg=MUTED, font=("Arial", 14, "bold"), anchor="w")
info_box_title.pack(anchor="w")
info_box = tk.Label(info_box_text, text="-", bg=CARD, fg=TEXT, font=("Arial", 12, "bold"), width=10, anchor="w")
...
capture_left = tk.Frame(body_frame, bg=CARD, highlightbackground=BORDER, highlightthickness=1, padx=6, pady=6, width=610)
...
# T54: Reserve a fixed bottom area for Reset Current Camera first.
# The preview then expands only into the remaining space and can never hide the button.
reset_button_wrap = tk.Frame(capture_left, bg=CARD, height=46)
reset_button_wrap.pack(side="bottom", fill="x", pady=(6, 0))
reset_button_wrap.pack_propagate(False)
...
preview_frame.pack(fill="both", expand=True, pady=(4, 4))
...
capture_state_label = tk.Label(preview_overlay, text="Waiting Object", bg=DARK, fg="white", font=("Arial", 10, "bold"), padx=2, pady=2)
capture_state_label.pack(side="left")

# Section 3 right status
capture_right = tk.Frame(body_frame, bg=CARD, highlightbackground=BORDER, highlightthickness=1, padx=9, pady=7)
capture_right.pack(side="right", fill="both", expand=True)

title_with_icon(capture_right, "status", "Progress", bg=CARD)
...
progress_title_label = tk.Label(product_header, text="Inner Box", bg=CARD, fg=TEXT, font=("Arial", 13, "bold"))
progress_title_label.pack(side="left")

product_row = tk.Frame(capture_right, bg=CARD)
product_row.pack(fill="x")
progress_label = tk.Label(product_row, text="0 / 0", bg=CARD, fg=TEXT, font=("Arial", 14, "bold"))
...
progress_bar_bg = tk.Frame(capture_right, bg="#c4c4c8", height=16)
...
tk.Label(check_header, text="Checklist", bg=CARD, fg=TEXT, font=("Arial", 13, "bold")).pack(side="left")
...
def make_check_item(name):
    item = tk.Label(checklist_frame, text=f"[ ]   {name}", bg=SOFT, fg=TEXT, font=("Arial", 12, "bold"), anchor="w", padx=18, pady=7, relief="flat")
...
# T43 UI CLEANUP: Step/Status details are already shown on the preview overlay.
# Keep widgets alive for set_step()/set_status(), but do not pack this box.
...
result_title = tk.Label(result_left_header, text="OCR Result", font=("Arial", 13, "bold"), bg=BG, fg=TEXT)
...
cancel_result_btn = add_action_button(result_button_frame, "Cancel Job", bg=RED, fg="white", command=cancel_job, width_px=135, height_px=42, frame_bg=BG)
restart_result_btn = add_action_button(result_button_frame, "Restart The Process", bg="#bfc3c7", fg=TEXT, command=restart_process, width_px=135, height_px=42, frame_bg=BG)
ok_next_btn = add_action_button(result_button_frame, "OK / NEXT", bg="#27ae60", fg="white", command=on_result_ok, width_px=135, height_px=42, frame_bg=BG)

result_hint = tk.Label(page_result, text="Review the OCR result, then press OK / NEXT to continue.", bg=BG, fg=MUTED, font=("Arial", 11))
# T43 UI CLEANUP: remove helper text line under OCR Result header.
# result_hint.pack(anchor="w", padx=18, pady=(0, 10))

product_result_area = tk.Frame(page_result, bg=BG)
product_result_area.pack(fill="both", expand=True, padx=4, pady=(0, 5))
...
toc_card = tk.Frame(page_ready_toc, bg=CARD, padx=46, pady=38, highlightbackground=BORDER, highlightthickness=1)
toc_card.place(relx=0.5, rely=0.48, anchor="center", relwidth=0.72, relheight=0.62)

# T43 UI CLEANUP: clearer transition page before CAM3 / Outer Box.
tk.Label(toc_card, text="✓", font=("Arial", 42, "bold"), bg=CARD, fg=GREEN).pack(pady=(0, 4))
tk.Label(toc_card, text="Inner Box Completed", font=("Arial", 13, "bold"), bg=CARD, fg=GREEN).pack(pady=(0, 10))
tk.Label(
    toc_card,
    text="Ready for Outer Box / TOC inspection",
    font=("Arial", 15, "bold"),
    bg=CARD,
    fg="#34495e",
    wraplength=520,
...
complete_card = tk.Frame(page_complete, bg=CARD, padx=28, pady=22, highlightbackground=BORDER, highlightthickness=1)
# V2.1 hotfix: give the Complete page enough vertical space so the OK button
# is always visible on the Station display. No other page/flow is changed.
complete_card.place(relx=0.5, rely=0.48, anchor="center", width=520, height=230)
complete_card.pack_propagate(False)

tk.Label(complete_card, text="TEST COMPLETED", font=("Arial", 24, "bold"), bg=CARD, fg=GREEN).pack(pady=(18, 24))
tk.Button(
    complete_card,
    text="OK",
    bg=BLUE,
